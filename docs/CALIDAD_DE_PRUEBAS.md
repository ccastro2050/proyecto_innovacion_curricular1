...
[Fact]
public async Task CrearAliado_Funciona()
{
    var creado = await servicio.CrearAliadoAsync(peticion);
    // (sin Assert: si CrearAliadoAsync guarda mal, esta prueba PASA)
}
```
...
```csharp
[Fact]
public async Task CrearAliado_GuardaElNombre()
{
    var creado = await servicio.CrearAliadoAsync(peticion);
