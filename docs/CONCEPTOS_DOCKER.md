...
- Viene de un **registro** o se construye localmente. Este proyecto usa de
  ...
  `api_innovacion/` (base: `dotnet/sdk:10.0`).
...
tiene 2: `./api_innovacion`, `./front_blazor`.
...
WORKDIR /app
ENV DOTNET_USE_POLLING_FILE_WATCHER=1 ASPNETCORE_URLS=http://0.0.0.0:8072
EXPOSE 8072
CMD ["dotnet", "watch", "run", "--non-interactive", "--project", "ApiInnovacion.csproj"]
```
...
| `EXPOSE 8072` | **Documenta** en qué puerto escucha el programa | No abre nada: quien publica el puerto es el `ports:` del compose |
...
| **Bind mount** | Una carpeta de SU disco montada dentro del contenedor | `./api_innovacion:/app` (el código entra al contenedor y `dotnet watch` lo vigila) · `./db:/scripts:ro` (los scripts del init, solo lectura) |
...
flowchart LR
    ...
    subgraph PC["Su PC — Docker Desktop (el 'centro de datos')"]
        subgraph RED["red interna del compose (LAN virtual, con DNS propio)"]
            FRONTBLAZOR["SERVIDOR WEB (front)<br/>contenedor front-blazor<br/>hostname: front-blazor · escucha en 8073"]
            APIINNOVACION["SERVIDOR DE APLICACIONES<br/>contenedor api-innovacion<br/>hostname: api-innovacion · escucha en 8072"]
    ...
    NAV -->|"localhost:8072"| APIINNOVACION
    NAV -->|"localhost:8073"| FRONTBLAZOR
    APIINNOVACION -->|"sqlserver:1433 (DNS de Docker)"| SQLSERVER
    FRONTBLAZOR -->|"http://api-innovacion:8072<br/>al controlador, por el NOMBRE"| APIINNOVACION
    SQLSERVERINIT -->|"espera el healthcheck,<br/>siembra y termina"| SQLSERVER
    CLIENTE -.->|"opcional (diagnóstico):<br/>localhost:11471"| SQLSERVER
```
...
```yaml
  sqlserver:
    ...
    ports:
      - "11471:1433"                 # "puerto en su PC : puerto interno"
...
```yaml
  sqlserver-init:
    ...
    volumes:
      - ./db:/scripts:ro             # init.sh + innovacion_local.sql, solo lectura
...
```yaml
  api-registros:
    build: ./api_innovacion            # se construye con SU Dockerfile
    volumes:
      - ./api_innovacion:/app          # guardar un .cs → dotnet watch recompila
...
1. **Dos redes de nombres**: hacia su PC, puertos publicados
   (`localhost:8072`, `localhost,11471`); entre contenedores, nombres de
...
docker network create proyecto_innovacion_curricular1_default

# 2. sqlserver
docker run -d --name innovacion-sqlserver --network proyecto_innovacion_curricular1_default --restart unless-stopped `
  -e "ACCEPT_EULA=Y" `
  -e "MSSQL_SA_PASSWORD=Aplicacionweb123!" `
  -v mssqldata:/var/opt/mssql `
  -p 11471:1433 mcr.microsoft.com/mssql/server:2022-latest

# 3. ESPERAR a que responda de verdad… mirándolo a ojo

# 4. sqlserver-init
docker run -d --name innovacion-sqlserver-init --network proyecto_innovacion_curricular1_default --restart no --entrypoint /bin/bash `
  -e "MSSQL_SA_PASSWORD=Aplicacionweb123!" `
  -v "${PWD}/db:/scripts:ro" mcr.microsoft.com/mssql/server:2022-latest /scripts/init.sh

# 5. Construir la imagen de api-innovacion y encenderla
docker build -t api-innovacion ./api_innovacion
docker run -d --name innovacion-api --network proyecto_innovacion_curricular1_default --restart unless-stopped `
  -e "ConnectionStrings__SqlServer=Server=sqlserver,1433;Database=innovacion_local;User Id=sa;Password=Aplicacionweb123!;TrustServerCertificate=True;" `
  -v "${PWD}/api_innovacion:/app" `
  -v /app/bin `
  -v /app/obj `
  -p 8072:8072 api-innovacion

# 6. Construir la imagen de front-blazor y encenderla
docker build -t front-blazor ./front_blazor
docker run -d --name innovacion-front --network proyecto_innovacion_curricular1_default --restart unless-stopped `
  -e "UrlApi=http://api-innovacion:8072" `
  -v "${PWD}/front_blazor:/app" `
  -v /app/bin `
  -v /app/obj `
  -p 8073:8073 front-blazor
...
```yaml
URL_API: http://api-innovacion:8072
```
...
|---|---|---|
| **Su navegador** | `http://localhost:8072` | Está fuera de Docker. Usa el puerto **publicado** |
| **Otro contenedor** | `http://api-innovacion:8072` | Están en la misma red: se llaman por el **nombre del servicio** |
...
> **En este proyecto los dos números son iguales** (`8072:8072`), y eso
...
Si alguien cambiara esa línea por `http://localhost:8072`, el front
...
```powershell
docker network inspect proyecto_innovacion_curricular1_default
```
...
`docker-compose.yml` y le pega `_default`. La carpeta `proyecto_innovacion_curricular1` produce
la red `proyecto_innovacion_curricular1_default`. Tres formas de confirmarlo:
...
network proyecto_innovacion_curricular1_default has active endpoints (name:"innovacion-front" …)
```
