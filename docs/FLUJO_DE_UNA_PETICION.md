...
| `[Route("api/aliado")]` | `AliadoController.cs` | "Todas mis rutas cuelgan de /api/aliado" |
...
| `[FromBody] AliadoCrear body` | parámetros del método | Captura el BODY: toma el JSON, lo vuelca en la PETICIÓN del verbo y lo VALIDA |

Cuando llega `GET /api/aliado`, el enrutador compara el verbo y la ruta
de la petición contra los atributos de todos los controladores, encuentra
el método `Listar()` (que tiene `[HttpGet]` bajo `[Route("api/aliado")]`)
...
1. ASP.NET routing     compara verbo+ruta contra los atributos
                       → elige AliadoController.Crear()
2. Model binding       toma el JSON del body y lo vuelca en la petición AliadoCrear
...
4. AliadoController  construye la entidad Aliado y llama al servicio
5. ServicioAliado    delega al repositorio (por la interfaz)
6. RepositorioAliadoSqlServer
                       INSERT INTO aliado (...) VALUES (@codigo, ...)  ← parametrizado
...
```mermaid
flowchart TD
    A["1. Kestrel recibe<br/>el texto HTTP"] --> B["2. ASP.NET Core encuentra<br/>POST /api/aliado<br/>en la tabla de rutas"]
    B --> C{"3. ¿el body cumple las<br/>anotaciones de AliadoCrear?<br/>(Required, Range...)"}
...
2. AliadoController try { … } → llama al servicio
3. ServicioAliado   pide al repositorio; si llega null → lanza
                      NoEncontradoExcepcion (el catch la vuelve 404)
4. Repositorio        SELECT ... WHERE codigo = @codigo
5. La fila vuelve convertida en objeto Aliado y sale como JSON
...
Invoke-RestMethod "http://localhost:8072/api/aliado"
Invoke-RestMethod "http://localhost:8072/api/aliado/PR001"

# POST — crear
Invoke-RestMethod -Method Post -Uri "http://localhost:8072/api/aliado" -ContentType "application/json" -Body '{"codigo":"PR009","nombre":"Webcam","stock":10,"valorunitario":350000}'

# PUT con body incompleto → error 422 (PUT exige TODO)
Invoke-RestMethod -Method Put -Uri "http://localhost:8072/api/aliado/PR009" -ContentType "application/json" -Body '{"stock":25}'

# PATCH con el MISMO body → 200 (PATCH es parcial)
Invoke-RestMethod -Method Patch -Uri "http://localhost:8072/api/aliado/PR009" -ContentType "application/json" -Body '{"stock":25}'

# DELETE — limpiar
Invoke-RestMethod -Method Delete -Uri "http://localhost:8072/api/aliado/PR009"
```
