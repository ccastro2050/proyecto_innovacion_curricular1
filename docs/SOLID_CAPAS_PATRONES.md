...
│ Controllers/AliadoController.cs                   │
...
│ Repositorios/RepositorioAliadoSqlServer.cs        │
│ El SQL a mano (Dapper): traduce filas ↔ objetos     │
│ Aliado. NO conoce HTTP. NO decide negocio.        │
└────────────────┬────────────────────────────────────┘
                 │  ④ SELECT … FROM aliado WHERE codigo = @codigo
                 ▼
          ┌───────────────┐
          │ BASE DE DATOS │  SQL Server — innovacion_local
          └───────┬───────┘
                  │
...
|---|---|---|---|
| **Controller** | HTTP: rutas, códigos de estado, JSON | SQL y reglas de negocio | `Controllers/AliadoController.cs` |
| **Servicio** | Las reglas del negocio (¿existe? ¿se puede?) | Saber de HTTP o del motor de BD | `Servicios/ServicioAliado.cs` |
| **Repositorio** | El SQL y el mapeo fila ↔ objeto | Saber de HTTP o decidir negocio | `Repositorios/RepositorioAliadoSqlServer.cs` |
...
Pregunta legítima: la carpeta de modelos (`Modelos/Aliado.cs`) existe en el
...
AGREGAR clases (`RepositorioAliadoPostgres : IRepositorioAliado`) y
...
public class RepositorioAliadoPostgres : IRepositorioAliado { /* … */ }
...
`RepositorioAliadoSqlServer` en las pruebas y el servicio ni se entera.

```csharp
// El repositorio FALSO de las pruebas (criterio 6): sin BD, misma interfaz
public class RepositorioFalsoEnMemoria : IRepositorioAliado
{
    private readonly Dictionary<string, Aliado> _datos = new();

    public Task<Aliado?> ObtenerPorCodigoAsync(string codigo)
        => Task.FromResult(_datos.GetValueOrDefault(codigo));
    // ...los otros 4 métodos...
}

// y el servicio NI SE ENTERA:
var servicio = new ServicioAliado(new RepositorioFalsoEnMemoria());
```

### I — Segregación de interfaces
Interfaces pequeñas y específicas: `IRepositorioAliado` tiene SOLO los 5
métodos de datos de aliado — no un "IRepositorioDeTodo" que obligue a
implementar métodos que no se usan.

```csharp
// ✅ La interfaz de la v1: SOLO los 5 métodos de datos de aliado
public interface IRepositorioAliado
{
    Task<List<Aliado>> ObtenerTodosAsync(int limite);
    Task<Aliado?> ObtenerPorCodigoAsync(string codigo);
...
```csharp
public ServicioAliado(IRepositorioAliado repositorio)  // ← interfaz, no clase
```
...
```csharp
// El contrato (Repositorios/IRepositorioAliado.cs):
Task<Aliado?> ObtenerPorCodigoAsync(string codigo);

// ServicioAliado lo usa SIN saber si detrás hay SQL Server, otro motor
...
builder.Services.AddScoped<IRepositorioAliado>(_ => fabrica.CrearRepositorioAliado());
...
(`RepositorioAliadoSqlServer`, el falso de las pruebas — y los
